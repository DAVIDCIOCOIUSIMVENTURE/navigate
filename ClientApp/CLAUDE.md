...
* The left sidebar was reinstated in July 2026 after an earlier removal. The shadcn primitive carries a custom tri-state `SidebarMode` API (`SidebarProvider` accepts `sidebarMode` / `onSidebarModeChange`; `toggleSidebar` cycles expanded → icon → collapsed).
