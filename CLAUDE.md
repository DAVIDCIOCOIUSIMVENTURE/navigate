...
There is no "Select a Problem" step: the problem is the project's own. `/projects/[projectId]/solutions/identify` redirects to the first step. The provider otherwise reads/writes the per-problem `SolutionWorkspace`. Back and Finish return to the project page. The pick-method step is labelled "Pick a method", matching the Research stepper. Solution candidates accumulated in this flow are written via `dispatch.solutions.create({ problemId, workspaceId, ... })`. The drawer `solutions-drawer.tsx` lists candidates for the current workspace.
