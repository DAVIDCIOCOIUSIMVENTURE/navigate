...
The provider only sets fields on the `Solution` (1-5 metric scores, validation notes/status/reason); the linked `Problem` is read-only here. It is a focus flow rendered through `FlowShell`: Back returns to the solution canvas (whose own Back returns to the project page), Finish on the last steps returns to the project page, the step nav card carries reminders of the solution and its linked problem with View Solution and View Problem buttons, and the journey rail sits on "Validate solutions".
