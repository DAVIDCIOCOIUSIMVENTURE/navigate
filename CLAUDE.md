...
| `notes` | `navigate-notes` | Journal notes (id/title/text/link/createdAt/editedAt). The journal is one list across the app, so every note is always visible. `link` is the area a note is about (`NoteLink`: general `none`, `self-discovery`, a project's `problem`, or one of its `solution`s by project and solution id). The editor picks it with two dropdowns: **Project** (No project, or a project) and **Section** (General or Self Discovery without a project; Problem or one of its solutions with one). The pure helpers behind them (the two Select values a link is shown as and read back from, the section options, the label shown on a note, the scope of the current page and the link a new note starts with) live in `src/lib/note-links.ts`: a note written inside a project starts on that project and its problem, or its solution when the page is about one, and one written in Self Discovery starts there. The panel offers "All notes" / "This project" (or "Self Discovery") filters while inside one of those areas. Created and updated timestamps are shown on every note. A note's `text` is **Markdown**, written in `MarkdownEditor` (`src/components/ui/markdown-editor.tsx`, Tiptap with its Markdown extension: bold, italic, two heading levels, bullet points and links from a toolbar, with the content styles under `.markdown-editor` in `globals.css`); the list preview strips the syntax with `plainTextFromMarkdown` (`src/lib/markdown-text.ts`). Reuse `MarkdownEditor` wherever free text wants basic formatting rather than adding another editor. Deleting a note confirms, then removes it. `projects.delete` calls `notes.unlinkProject`, which makes that project's notes general; `solutions.delete` calls `notes.unlinkSolution`, which moves that solution's notes up to the project's problem. Storage is validated on read (`parseStoredNotes`), so a note saved before links existed comes back general and one saved with a bare `projectId` comes back linked to that project's problem. |
...
* `src/app/root-layout-client.tsx`: Client layout containing:
  ...
  * Top header bar spanning the full width. Left side: Navigate logo, sidebar trigger, breadcrumb. Right side: the projects menu (`src/components/projects-menu.tsx`: a white dropdown button showing the current project's name, or "Projects"; its list opens with "New project", which raises `NewProjectDialog` from `src/components/project-name-dialog.tsx`, then every project with a tick on the current one and a settings button opening `ProjectSettingsDialog`), admin panel link, journal/guidance panel toggles (flat borderless icon buttons, transparent at rest, white with `text-quaternary` while their panel is open), account menu. The same projects menu sits in the top sheet that focus flows reveal. **The header never shows team avatars**: a team belongs to one project, so its members are drawn on the project page's left column, in the projects table and in `ProjectSettingsDialog`, never in the app-wide chrome.
  * **The admin panel is a separate space** (`isAdminPath` in `root-layout-client.tsx`, tested beside `isFocusFlowPath`: `/admin` and everything under it). It keeps the header with the logo, breadcrumb, admin link and account menu, but nothing that belongs to the user's own work is drawn there: no left sidebar or sidebar trigger, no projects menu, no journal or guidance toggles, and a journal or guidance panel left open elsewhere is not shown (the store keeps its state, so it is back when the user leaves the panel). The breadcrumb's Home link is the way out.
  * A left sidebar (`src/components/app-sidebar.tsx`, built on the shadcn `sidebar` primitive in `src/components/ui/sidebar.tsx`) sits **below** the header. It lists the top-level sections from `navigationItems.sidebar` (Home, Why It Matters, Self Discovery, Projects, Next Steps; Problems and Solutions were dropped when projects arrived, Portfolios when the project preview replaced it, and Projects was added in September 2026 when Home became an overview). It has three modes cycled by the header trigger (or Ctrl/Cmd+B): `expanded` (full labels), `icon` (icons only, labels as tooltips), `collapsed` (hidden off-canvas). The mode is persisted as `settings.sidebarMode`. On mobile it renders as a Sheet. It is hidden in full view and focus flows, along with the header. Focus flows are listed in `isFocusFlowPath` in `root-layout-client.tsx` (tested in `root-layout-client.test.ts`): Self Discovery and every page under a project (the Identify problems hub and its tools, the problem edit page and its Explore and Validation flows, the solution canvas, its edit page and its validation flow, Identify Solutions and Compare solutions); each supplies its own chrome buttons and section title. **The project page itself (`/projects/<id>`) is not a focus flow**: opening a project must always show the top menu, so it keeps the header. It does drop the left sidebar and the header's sidebar trigger (`hidesSidebarPath`, beside `isFocusFlowPath`), because its own left column takes the sidebar's place. **Those two buttons are always `FocusChromeButtons`** (`src/components/focus-chrome-buttons.tsx`): "Home", which leaves for `/` however deeply nested the page is, and "Menu", the `PanelTop` button that calls `revealTopNav` (its accessible name and tooltip are the fuller "Open menu"). There is no per-page destination to pass and no Back button anywhere in the chrome: a flow never walks out one level at a time. In-page step buttons labelled "Back" (the Canvas Builder's Builder mode, the flow steppers) are a different thing and still walk the steps, and the pages that offer a "Back to the project" button in their own content (the solution canvas and edit pages' not-found cards) still do. Flows without a stepper rail (the hubs, the Canvas Builder) render that row with `FocusFlowHeader` (`src/components/focus-flow-header.tsx`). Pages about one project, problem or solution (the project page, the solution canvas and both edit pages) place the chrome buttons and the journey rail in a left column beside their content with `FocusPageShell` (`src/components/focus-page-shell.tsx`), which takes `title` and `icon` rather than a header node; the project page passes it `chrome="app"` because it keeps the app header, so the shell skips the page padding the layout already gives it, caps the column height under the header and draws no chrome buttons, so Home and the menu are not drawn twice. **On wide containers the section title lives inside the column's first card**, never above it: the step nav card where the page has a stepper (`FlowShell`, `LensStepper`, `ResearchStepper`, the Identify Solutions and Self Discovery layouts, all through `CardSectionTitle` in `src/components/section-title.tsx`), otherwise at the head of the journey rail card (`JourneyProgressCard`'s `header`, which `FocusPageShell` and `IdentifyHubShell` use). Icon-only buttons that belong to the title (the hub's About button, the project page's Settings button) sit at the right of that row. On narrow containers the title stays in the header row with the chrome buttons (`FocusFlowHeader`, or `SectionTitle` from the same file), because the collapsed nav there is a dropdown rather than a card. Because the sidebar panel is fixed-positioned, `app-sidebar.tsx` offsets it below the `h-16` header with `!top-16 h-[calc(100svh-4rem)]`; recompute if the header height changes.
  ...
  * `getCrumbs(pathname, lookup)` (exported and tested in `root-layout-client.test.ts`) derives the breadcrumb shown in the header from the route segments; `/projects` reads "Home / Projects" and under `/projects/<id>/...` it reads "Home / Projects / <project> / Explore" (or Test, Edit problem, Identify a problem / Canvas Builder, Identify solutions, Compare solutions, Solution / Test), with `lookup` resolving the project's display name. **When adding a new top-level route, update `getCrumbs` here as well as `src/config/navigation.ts`.**
* **Home (`/`)** is the overview: a Self Discovery card (the progress ring, how many questions are answered, "Start" or "Continue Self Discovery" into the questionnaire and "Your answers" to the library page), two quick-link cards into Why It Matters and Next Steps (drawn like the section cards described under "Page titles and section navs"), and the projects in `ProjectsTable`, whose card header carries, after the search box, the "New project" button that the guided tour anchors to (the empty state's own button carries the same anchor while there are no projects) and an "All projects" link to the Projects page. **Page titles sit inside the page's card**, not above it: Home keeps its title row because it has several cards, but Projects, Self Discovery and the Why It Matters and Next Steps nav columns pass their `CardTitle`, About button and actions into the card's `CardHeader` (`ProjectsTable` takes them as `title` / `headerExtra`), so the card is the whole page or column. **Projects (`/projects`)** is the list on its own, and where a page about a project that no longer exists sends the user (`PROJECTS_HREF` / `projectRoutes.list()` in `src/lib/projects.ts`; the gates in `project-gates.tsx`, the project page's not-found card and its `onDeleted` all go there). It lists projects in `ProjectsTable` (`src/components/projects-table.tsx`: name and problem title, the problem's status, solution count, team avatars and a settings button; newest project first by default, the whole row opens the project, and the settings button is the row's only other control, because a project's portfolio, its team and the way to delete it all live in `ProjectSettingsDialog`) with a "New project" button and the bundle import/export menu (`BundleMenuButton`, labelled "Import project" / "Export project"; an imported project appears as a new row and the page stays put, see "Export and import" below). Both pages show `ProjectsEmptyState` (`src/components/projects-empty-state.tsx`) while there are no projects. **The project page (`/projects/[projectId]`)** keeps the app header, unlike the flows it leads into: opening a project always shows the top menu. The left sidebar is hidden there (`hidesSidebarPath` in `root-layout-client.tsx`) so the project's own left column is the only one. It is built on `FocusPageShell` in `chrome="app"` mode: the left column is a single card (no Home or Open menu buttons, which the header already provides) headed by the project's name with an icon-only Settings button at its right opening `ProjectSettingsDialog` (where the team, "Open portfolio" and delete all live; the column draws no team avatars), above the journey rail on the problem's own step (`problemJourneyStep`). Beside it, scrolling together as one column, sit the `ProblemCanvas` (Explore, Validate, Edit, Full view, Download as text, Export project, all in one actions menu behind a gear button beside the status pill) in a viewport-height box on wide containers, and a `SolutionsTable` of the project's solutions with "Identify solutions" and "Compare solutions" (see "Solutions guards" below: neither is ever disabled). A project without a problem shows a "No problem yet" card whose "Identify a problem" button opens the project's hub (`/projects/<id>/identify`), and the tools there pass the project id into `problems.create` so the problem lands in it. `/problems/<id>` only redirects to the owning project; `/problems` and `/solutions` redirect to `/`.
...
* **Section navs and steppers** take their classes from `src/lib/nav-item-styles.ts` rather than inlining them:
  ...
  * `PAGE_TITLE_CLASS` on the `size="md"` `CardTitle` that heads each top-level page (Home, Why It Matters, Self Discovery, Projects, Next Steps), so all five share one size, in the title's own bold weight but the default foreground colour rather than primary; do not restate the classes at the call site.
  * `SECTION_TITLE_TILE_CLASS` / `SECTION_TITLE_ICON_CLASS` for the compact `h1` tile used by focus flows (Self Discovery, Identify Solutions, the guided prompt tools, Research, and the Identify problems hub via `src/components/identify-hub-shell.tsx`). Do not build that `h1` by hand: `SectionTitle` in `src/components/section-title.tsx` is the heading, and `CardSectionTitle` is the same heading at the head of a left-column card, above a rule, with room for an icon button at the right (cards padded `p-3` pass `px-3 pt-1.5` so the tile lines up with the rows below).
...
* **In-page tiles** that are not the page title (the numbered intro steps, the introduction-page step timelines, the empty-state icon) also use the solid `bg-secondary-brand` tile with `text-secondary-brand-foreground`, and the title beside each tile is `text-secondary-brand`, so primary is reserved for the page heading and the active nav row.
* **Section cards** (the "Browse the sections" cards on Why It Matters, the "Where to go from here" cards on Next Steps, the question cards on a Self Discovery category) go further: the whole card is a solid `bg-secondary-brand` button with white text, chevron and tagline (`hover:bg-secondary-brand/90`), and the icon or number sits inverted on a white tile in `text-secondary-brand`. Copy one of those three when adding a list of cards that lead into a section.
...
* The Identify problems hub gets it through `IdentifyHubShell`'s `journeyStep` prop. On wide containers the shell builds a left column (`FOCUS_COLUMN_WIDTH_CLASS` from `flow-shell.tsx`, the width every focus flow's column shares; widen it there, not per page) holding the chrome buttons (Home, Open menu) above the rail card, which is headed by the title and the About icon, with the content card beside it; on narrow ones the header row (chrome buttons, title, About) stays on top and the rail becomes a horizontal row above the card. The guided prompt and Research layouts place the same card directly under their stepper (vertical in the wide side column, horizontal below the collapsed stepper on narrow), still on the "Identify problem" step. The Identify Solutions layout does the same on the "Identify solutions" step. Reuse those arrangements rather than inventing another placement.
...
### Strategy and case-study tabs

Every hands-on step that pairs the user's own work with worked examples (the Explore and Test the problem steps, the solution metric and verdict steps, the four Identify Solutions methods, the Guided discovery picker questions) draws the pair with `StrategyTabs` (`src/components/strategy-tabs.tsx`): `strategy` is the cobalt card the user works in, `children` the read-only case studies, and the labels default to "Your Strategy" / "Case Studies" (Guided discovery passes "Your answer" / "Inspiration"). The active tab is attached to the card below it like the tab on a folder: it takes the card's colour (cobalt for the strategy, the case-study panel colour for the case studies) and sits flush on the card's top edge, while the other tab waits in `bg-muted` grey beside it. Case-study panels are drawn with `CASE_STUDY_PANEL_CLASS` from the same file (the `bg-border` tone, a shade darker than the idle tab, so the open panel and its tab tell apart from the tab that is waiting), with the gap added at the call site. Do not hand-roll the pair with `Tabs` again, and do not put a display class on a `TabsContent` (it would override the `hidden` attribute Radix gives the inactive panel and leave an empty panel taking space); the component keeps the inner case-study tabs (one per company) as they are.
...
* `/`: Home, the overview of Self Discovery and the projects with quick links into the reading sections (see "Layout & Navigation Patterns")
* `/projects`: The list of projects, with the import and export menu
...
* `/admin`, `/admin/users/[userId]`, `/admin/projects/[projectId]`: Admin panel (establishments, classes, users, projects). The Projects tab (`src/components/admin-projects-table.tsx`) lists every project in this browser's storage, since there is no backend yet to narrow it to the projects the admin may see, and each row opens `/admin/projects/<id>` (`adminProjectHref` in `src/lib/projects.ts`): the project's portfolio rendered inside the app with `ProjectPreviewBody`, plus the team's journal notes, which the public portfolio never shows. The notes sit in a margin down the right of the portfolio like the comments in a word processor (`NotesMargin` in `project-preview.tsx`: a sticky `lg:w-72` column beside the content on wide containers, stacked below it on narrow ones, reserved on every row so the content column keeps one width). `projectNotesBySection` in `src/lib/note-links.ts` decides where each note goes: a note linked to one of the project's solutions sits beside that solution's article ("Notes on this solution"), and a note linked to the problem, or to a solution that no longer exists, sits beside the problem sections ("Notes on the project"). Every note is a `NoteComment` collapsed to its heading (`noteHeading`: the title, else the first line of the text) and dates; clicking opens the full note, and the first margin carries "Expand all" / "Collapse all" for the whole page. Note text is Markdown, drawn read only by `MarkdownView` (`src/components/ui/markdown-view.tsx`, the editor's extensions and styles with editing off), mounted only while the note is open.
