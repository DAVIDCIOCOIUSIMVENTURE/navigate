...
* **Home (`/`)** lists projects in `ProjectsTable` (`src/components/projects-table.tsx`: name and problem title, the problem's status, solution count and team avatars; the whole row opens the project, and there are no per-row buttons, because a project's portfolio, its settings and the way to delete it all live in `ProjectSettingsDialog` inside the project) with a "New project" button and the bundle import/export menu (`BundleMenuButton`, labelled "Import project" / "Export project"; an imported project appears as a new row and the page stays put, see "Export and import" below). **The project page (`/projects/[projectId]`)** is a regular page (header and sidebar visible): one title row holding the name, the team avatars, the horizontal journey rail on the problem's own step (`problemJourneyStep`, the bare `JourneyProgress` rather than its card) and a single Settings button opening `ProjectSettingsDialog` (where "Open portfolio" and delete both live), then the `ProblemCanvas` (Explore, Validate, Edit, Full view, Download as text, Export project, all in one actions menu behind a gear button beside the status pill) inside a viewport-height box on wide containers, and a `SolutionsTable` of the project's solutions with "Identify solutions" and "Compare solutions" (see "Solutions guards" below: neither is ever disabled). A project without a problem shows a "No problem yet" card whose "Identify a problem" button opens the project's hub (`/projects/<id>/identify`), and the tools there pass the project id into `problems.create` so the problem lands in it. `/problems/<id>` only redirects to the owning project; `/problems` and `/solutions` redirect to `/`.
