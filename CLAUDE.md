...
| `projects` | `navigate-projects` | The user's projects (`id`, `name`, `problemId | null`, timestamps) and a `hydrated` flag. See "Projects" below. |
...
* **Home (`/`)** lists projects in `ProjectsTable` (`src/components/projects-table.tsx`: name and problem title, the problem's status and solution count, with Open / Rename / Delete per row) with a "New project" button and the bundle import/export menu (`BundleMenuButton`, labelled "Import project" / "Export project": a problem bundle is a whole project, and an imported one opens its project page). **The project page (`/projects/[projectId]`)** is a regular page (header and sidebar visible): title row with Rename and Delete, the horizontal journey rail on the problem's own step (`problemJourneyStep`), then the `ProblemCanvas` (Explore, Validate, Edit, Download, Export, Full View) inside a viewport-height box on wide containers, and a `SolutionsTable` of the project's solutions with "Identify solutions" (enabled once the problem is Valid or Unsure, calling `startIdentifySolutions(push, problemId)`) and "Compare solutions". A project without a problem shows a "No problem yet" card whose "Identify a problem" button opens the project's hub (`/projects/<id>/identify`), and the tools there pass the project id into `problems.create` so the problem lands in it. `/problems/<id>` only redirects to the owning project; `/problems` and `/solutions` redirect to `/`.
