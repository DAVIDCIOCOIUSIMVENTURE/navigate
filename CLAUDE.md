...
| ----- | ---------------- | ------- |
| `settings` | `navigate-settings` | Full-view toggle, journal panel open state. (Also stores a legacy `sidebarMode` field that is no longer read; the left sidebar has been removed.) |
...
* The shadcn `sidebar` UI primitive (`src/components/ui/sidebar.tsx`) is no longer mounted in the app shell. The previous `AppSidebar` component has been removed. Do not reintroduce a left sidebar without explicit direction.
