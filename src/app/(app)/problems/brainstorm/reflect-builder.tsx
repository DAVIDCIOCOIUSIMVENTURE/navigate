...
function Stepper({
  ...
  const progressBar =
    activeId === "prompts" && promptsProgress ? (
      <div
        className="h-1.5 w-full rounded-full bg-muted overflow-hidden"
        role="progressbar"
        aria-valuemin={0}
        aria-valuemax={promptsProgress.total}
        aria-valuenow={promptsProgress.current}
      >
        <div
          className="h-full bg-primary transition-all"
          style={{ width: `${(promptsProgress.current / promptsProgress.total) * 100}%` }}
        />
      </div>
    ) : null
