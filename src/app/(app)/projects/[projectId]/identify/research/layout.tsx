...
export default function ResearchLayout({ children }: { children: ReactNode }) {
  ...
  // project's own. When the project already has its problem, the research it
  // was captured with is pre-filled and saving updates that problem.
  ...
  const activeToolId = activeMethod ? projectResearch.sessions[activeMethod.id]?.toolId ?? null : null
  ...
  useEffect(() => {
    ...
    const href = researchResumeHref(projectId, storedMethodId, storedStep, storedPromptIndex)
    if (!href) return
    resumeHrefRef.current = href
    router.replace(href)
  }, [restored, hydrated, route.kind, storedMethodId, storedStep, storedPromptIndex, router, projectId])
  ...
  }, [restored, isResuming, route.kind, storedMethodId, storedPromptIndex, dispatch, projectId])
  ...
  const handleStepClick = useCallback(
    (id: ResearchStep) => {
      ...
      router.push(hrefs.step(id, activeMethod.id, 0))
    },
    [router, activeMethod, isStepEnabled, hrefs],
  )

  const handleReset = useCallback(() => {
    dispatch.researchSessions.clearProject(projectId)
    router.push(hrefs.pick())
  ...
  const stepper = (
    <ResearchStepper
      ...
      onReset={handleReset}
      resetDescription="This will return you to the method picker and clear in-progress capture answers. The problem already saved in this project is not affected."
  ...
  return (
    <div className="flex flex-1 min-h-0 w-full flex-col">
      ...
      >
        {routeMethod ? (
          <ResearchProvider projectId={projectId} method={routeMethod} seed={seed}>
            {inner}
          </ResearchProvider>
        ) : inner}
      </div>
