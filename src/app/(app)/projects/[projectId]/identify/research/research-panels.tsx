...
import { saveResearchCapture } from "@/lib/research-capture"
    ...
    try {
      ...
      if (existing) {
        dispatch.problems.update({
          id: existing.id,
          patch: { title: trimmedTitle, customers: customerIds, problems: problemIds },
        })
        saveResearchCapture(existing.id, research)
        clearSession()
      ...
      saveResearchCapture(newProblem.id, research)
    ...
    <div className="flex flex-col gap-6 w-full flex-1 min-h-0 overflow-y-auto">
      ...
      <p className="text-base leading-relaxed">
        {hasCandidate ? (
          <>
            {existing ? (
              <>Saving will <span className="font-semibold">update your project&apos;s problem</span> with this research.</>
            ) : (
              <>Your research will be saved as your project&apos;s <span className="font-semibold">problem</span>.</>
            )}{" "}
            Click any heading to jump back to that step.
          </>
        ) : (
