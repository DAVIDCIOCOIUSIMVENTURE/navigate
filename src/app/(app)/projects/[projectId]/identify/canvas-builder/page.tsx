...
import { dimensionColumns, type DimensionItem, type DimensionColumn } from "@/app/(app)/problems/identify/data"
...
import { isCanvasDraftEmpty, selectCanvasDraft, type CanvasDraft } from "@/store/canvas-drafts-model"
...
function ProblemBuilder({
  ...
  /** The project whose draft the builder edits. */
  projectId: number
  columns: DimensionColumn[]
  onSave: (selections: Record<string, string[]>, description: string) => void
  /** True when the project already has its problem, so saving updates it rather than creating one. */
  updating?: boolean
  onClearSearch?: () => void
  customItemColumns: Map<string, string>
}) {
  const dispatch = useDispatch<AppDispatch>()
  // The builder walks the same per-project draft the canvas mode ticks, so
  // toggling an item in either mode is reflected in the other.
  const draft = useSelector((state: RootState) => selectCanvasDraft(state, projectId))
  const { step, activeColumnId, activeCategoryId, title: problemTitle } = draft
  const selectedSet = useMemo(() => new Set(draft.selected), [draft.selected])
  const isWide = useContainerSize() === "wide"
  const [stepperOpen, setStepperOpen] = useState(false)

  const patchDraft = (patch: Partial<CanvasDraft>) => dispatch.canvasDrafts.update({ projectId, patch })
  const reset = () => dispatch.canvasDrafts.clearProject(projectId)
  const setStep = (next: BuilderStepId) => patchDraft({ step: next })
  const setActiveColumnId = (id: string | null) => patchDraft({ activeColumnId: id })
  const setActiveCategoryId = (id: string | null) => patchDraft({ activeCategoryId: id })
  const setProblemTitle = (next: string) => patchDraft({ title: next })
  ...
  const toggleItem = (_columnId: string, itemId: string) => {
    ...
    patchDraft({ selected: Array.from(next) })
  }

  // Saving hands the draft over; the page clears it once the problem is stored.
  const handleSave = () => {
              ...
              <div className={cn("flex flex-col gap-4 min-w-0", isWide && "flex-1")}>
                ...
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <Button size="sm" onClick={handleSave} disabled={totalSelections === 0} className="gap-2">
                    ...
                    {updating ? "Update Problem" : "Save Problem"}
                  </Button>
                </div>
              </div>
            </div>
...
export default function IdentifyPage() {
  ...
  // The canvas ticks the project's own draft, which the Builder mode walks step by step.
  const { projectId, problem: existing } = useProjectScope()
  const draftsHydrated = useSelector((state: RootState) => state.canvasDrafts.hydrated)
  const draft = useSelector((state: RootState) => selectCanvasDraft(state, projectId))
  ...
  const setSelected = (updater: Set<string> | ((prev: Set<string>) => Set<string>)) => {
    ...
    dispatch.canvasDrafts.update({ projectId, patch: { selected: Array.from(next) } })
  }
  const resetDraft = useCallback(() => dispatch.canvasDrafts.clearProject(projectId), [dispatch.canvasDrafts, projectId])
  ...
  // The canvas runs inside a project. A project holds one problem, so when
  // it already has one the canvas opens with that problem's selections and
  // title, and saving updates the problem instead of creating another.
  const router = useRouter()
  const existingId = existing?.id ?? null
  // Revisiting the project's problem starts from what it captures: an empty
  // draft is filled from the problem once per visit, while a draft the user
  // has already changed is left alone.
  const seededForRef = useRef<number | null>(null)
  useEffect(() => {
    if (!existing || !draftsHydrated || seededForRef.current === existing.id) return
    seededForRef.current = existing.id
    if (!isCanvasDraftEmpty(draft)) return
    const selected = [...existing.customers, ...existing.contexts, ...existing.problems, ...existing.you]
    if (selected.length === 0 && existing.title.trim().length === 0) return
    dispatch.canvasDrafts.update({ projectId, patch: { selected, title: existing.title } })
  }, [existing, draftsHydrated, draft, dispatch.canvasDrafts, projectId])

  const finishUpdate = useCallback(() => {
    resetDraft()
    toast.success("Problem updated.")
    router.push(projectRoutes.page(projectId))
  }, [resetDraft, router, projectId])

  /**
   * After saving, the project has the problem the draft became, so the
   * seeding effect above would read the draft back out of it. Mark it as
   * already seeded so the cleared draft stays cleared.
   */
  const finishCreate = useCallback((problemId: number) => {
    seededForRef.current = problemId
    resetDraft()
    setLastSavedProblemId(problemId)
  ...
  const handleBuilderSave = useCallback(async (selections: Record<string, string[]>, problemTitle: string) => {
    ...
    if (existingId !== null) {
      dispatch.problems.update({ id: existingId, patch: { ...patch, title: problemTitle } })
      finishUpdate()
      return
    }
    const newProblem = await dispatch.problems.create({ ...patch, source: "identify", title: problemTitle, projectId })
    finishCreate(newProblem.id)
  }, [allColumns, dispatch.problems, existingId, finishCreate, finishUpdate, projectId])
  ...
  const saveCombination = async () => {
    ...
    if (existingId !== null) {
      dispatch.problems.update({ id: existingId, patch: { ...patch, title: saveTitle.trim() } })
      setSaveDialogOpen(false)
      finishUpdate()
      return
    }
    const newProblem = await dispatch.problems.create({ ...patch, source: "identify", title: saveTitle.trim(), projectId })
    setSaveDialogOpen(false)
    finishCreate(newProblem.id)
  }
      ...
      <ConfirmDialog
        ...
        description={
          canvasBuilderMode === "canvas"
            ? "This will clear your current selection across all dimensions. A problem you have already saved is not affected."
            : "This will clear your in-progress problem builder. A problem you have already saved is not affected."
        }
        confirmLabel="Reset"
        onConfirm={() => {
          // Both modes work on the same draft.
          clearAll()
        }}
      />
      {canvasBuilderMode === "canvas" && (
        ...
        >
          <Save className="h-3.5 w-3.5" />
          <span className={cn(containerSize === "narrow" && "sr-only")}>{existing ? "Update Problem" : "Save Problem"}</span>
        </Button>
      )}
    </>
  )
  ...
  const content = (
    ...
    >
      <FocusFlowHeader title="Canvas Builder" icon={Brain} backHref={projectRoutes.identify(projectId)} />
      {existing && (
    ...
    <div className="flex flex-col gap-3 w-full flex-1 min-h-0 min-w-0 overflow-x-hidden">
      {containerSize === "wide" ? (
        ...
        <div className="flex flex-col items-stretch gap-3 min-w-0 shrink-0">
          <div className={cn(
            "flex min-w-0 items-center gap-2",
            containerSize === "narrow" && "justify-between",
          )}>
            {toggleGroupEl}
      ...
      {canvasBuilderMode === "builder" ? (
        <ProblemBuilder projectId={projectId} columns={filteredColumns} onSave={handleBuilderSave} updating={existing !== undefined} onClearSearch={() => setSearchQuery("")} customItemColumns={customItemColumns} />
      ) : (<>
      <div className={cn(
      ...
      <ProblemFormDialog
        ...
        title={existing ? "Update Problem" : "Save Problem"}
        ...
        actions={
          <>
            ...
            <Button onClick={saveCombination}>{existing ? "Update Problem" : "Save Problem"}</Button>
          </>
        }
      />
