...
export function MetricStep({ content, value, onChange }: MetricStepProps) {
  ...
  const { projectId, solutionId } = useSolution()
  const { prevPath, nextPath } = getAdjacentSteps(pathname, projectId, solutionId)
      ...
      <CardContent className="p-10 pt-6 flex flex-col gap-6">
        ...
        <div className="flex justify-between mt-2">
          ...
          {nextPath ? (
            ...
            <Button onClick={() => router.push(finishHref)}>Finish</Button>
          )}
        </div>
      </CardContent>
    </Card>
