    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Refine the solution before adding it to the project. You can edit it again later from the project page.
          </DialogDescription>
        </DialogHeader>
...
function SaveSolutionPanel({ toolType }: { toolType: SaveDialogTool }) {
  ...
  const handleContinue = () => {
    ...
    if (savedSolutionId !== null) {
      router.push(projectRoutes.solutionValidate(projectId, savedSolutionId))
    }
  }
...
export default function DiscoverPage() {
  ...
  const { projectId, problemId, discoveryToolType } = useIdentifySolutions()
  const { prevPath, nextPath } = getAdjacentSteps(pathname, projectId)
  const containerSize = useContainerSize()
  const isNarrow = containerSize === "narrow"

  const finishHref = projectRoutes.page(projectId)
      ...
      <CardContent className="p-10 pt-6 flex flex-col gap-6">
        ...
        {!discoveryToolType && (
          <div className="flex flex-col items-center justify-center gap-3 py-8 rounded-lg border border-dashed">
            ...
            <Button variant="outline" onClick={() => guardedRouter.push(projectRoutes.identifySolutions(projectId))}>
        ...
        <div className="flex justify-between mt-2">
          ...
          {nextPath ? (
            ...
            <Button onClick={() => guardedRouter.push(finishHref)}>
