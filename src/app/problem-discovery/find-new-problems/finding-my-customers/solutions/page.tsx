...
export default function SolutionsPage() {
  ...
  function addSolution(jobId: number) {
    dispatch.findingMyCustomers.addSolution(jobId)
  }

  function updateSolution(jobId: number, id: number, text: string) {
    dispatch.findingMyCustomers.updateSolution({ jobId, id, text })
  }

  function removeSolution(jobId: number, id: number) {
    dispatch.findingMyCustomers.removeSolution({ jobId, id })
  }
  ...
  return (
    <Card className="w-full flex-1">
      <CardContent className="p-8 flex flex-col gap-4">
        ...
        <p className="text-sm">
          Survey the existing solutions your customers currently use. Understanding what is already available —
          and where those solutions struggle — helps you pinpoint the gaps that represent real problems worth solving.
        </p>
