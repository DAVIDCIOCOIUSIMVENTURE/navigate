...
export default function ProblemsPage() {
  ...
  function addProblem(jobId: number, solutionId: number) {
    dispatch.findingMyCustomers.addProblem({ jobId, solutionId })
  }

  function updateProblem(jobId: number, solutionId: number, id: number, text: string) {
    dispatch.findingMyCustomers.updateProblem({ jobId, solutionId, id, text })
  }

  function removeProblem(jobId: number, solutionId: number, id: number) {
    dispatch.findingMyCustomers.removeProblem({ jobId, solutionId, id })
  }
  ...
  return (
    <Card className="w-full flex-1">
      <CardContent className="p-8 flex flex-col gap-4">
        ...
        <p className="text-sm">
          Capture and articulate the specific problems you have uncovered through your customer analysis.
          For each existing solution your customers use, identify the problems or frustrations it fails to address.
        </p>
